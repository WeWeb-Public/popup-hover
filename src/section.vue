...
<!-- This is your HTML -->
<template>
  <div v-show="display" class="popup-hover">
    <!-- Weweb Wallpaper -->
    <wwObject
      class="background"
      v-bind:ww-object="section.data.color"
    ...
    <div class="popup-container">
      ...
      <wwObject
        class="close"
        v-bind:ww-object="section.data.close"
        @click="close"
      ></wwObject>
      <wwLayoutColumn
        tag="div"
        ww-default="ww-text"
        :ww-list="section.data.list"
        class="list"
        @ww-add="add(section.data.list, $event)"
        @ww-remove="remove(section.data.list, $event)"
      >
        <wwObject
          tag="div"
          v-for="object in section.data.list"
          :key="object.uniqueId"
          :ww-object="object"
        ></wwObject>
      </wwLayoutColumn>
      <wwObject
        class="close-text"
        v-bind:ww-object="section.data.closeText"
        @click="close"
      ></wwObject>
    </div>
  </div>
...
export default {
  ...
  methods: {
    init() {
      ...
      this.section.data.timeBeforeDisplaying =
        this.section.data.timeBeforeDisplaying || 3;
    ...
    addCookie() {
      ...
      this.$cookies.set("popup-hover-closed", true, {
        expires: in24h,
        path: "/",
      });
    },
